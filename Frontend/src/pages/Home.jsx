      ...
      <div className="relative overflow-hidden rounded-2xl bg-[#111] border border-white/5 group-hover:border-violet-500/40 transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-violet-900/20">
        <div className="relative overflow-hidden aspect-[3/4]">
          <img
            ...
            alt={product.name}
            className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 ${outOfStock ? "opacity-50 grayscale" : ""}`}
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {isNew && !outOfStock && <span className="px-2.5 py-1 bg-violet-600 text-white text-[9px] font-black tracking-widest uppercase rounded-full">NEW</span>}
            {outOfStock && <span className="px-2.5 py-1 bg-black/80 text-white/50 text-[9px] font-black tracking-widest uppercase rounded-full border border-white/10">SOLD OUT</span>}
            {!outOfStock && product.stock_quantity <= 5 && <span className="px-2.5 py-1 bg-amber-500/90 text-black text-[9px] font-black tracking-widest uppercase rounded-full">LAST {product.stock_quantity}</span>}
          </div>
        </div>
        <div className="p-4 pb-5">
          <p className="text-violet-400/70 text-[10px] tracking-widest uppercase font-bold mb-1">{product.category}</p>
          <h3 className="text-white/90 text-sm font-bold leading-snug mb-3 group-hover:text-white transition-colors line-clamp-2">{product.name}</h3>
          <div className="flex items-center justify-between">
            <span className="text-white font-black text-base tracking-tight">ZMK {product.price?.toLocaleString()}</span>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(124,58,237,0.25) 0%, transparent 70%)" }} />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-16 sm:pt-28 sm:pb-20">
          <div className="max-w-3xl">
            ...
            <h1 className="text-white font-black tracking-tighter leading-[0.9] mb-6" style={{ fontSize: "clamp(3rem, 8vw, 6rem)" }}>
              WEAR YOUR<br />
              <span className="text-transparent bg-clip-text" style={{ backgroundImage: "linear-gradient(135deg, #7c3aed 0%, #a78bfa 50%, #7c3aed 100%)" }}>ANIME</span><br />
              IDENTITY
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <a href="#catalog" className="px-8 py-4 bg-violet-600 hover:bg-violet-500 text-white font-black text-sm tracking-wide uppercase rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-violet-600/30 hover:-translate-y-0.5">
                Shop Now
              </a>
              <a href="https://wa.me/+260978917953?text=Hi! I'd like to order." target="_blank" rel="noreferrer"
          ...
          <div className="flex gap-3 mt-14 overflow-x-auto pb-2" style={{scrollbarWidth:"none",msOverflowStyle:"none"}}>
            {[
              ...
              <div key={stat.label} className="flex items-center gap-2.5 px-4 py-2.5 bg-white/[0.04] border border-white/8 rounded-full backdrop-blur-sm shrink-0">
      ...
      <section id="catalog" className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 mb-8">
          <div>
            ...
            <h2 className="text-white text-2xl sm:text-3xl font-black tracking-tight">
              ...
              <span className="text-white/20 font-medium text-lg ml-3">{filteredProducts.length}</span>
            </h2>
          </div>
        ...
        <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-1" style={{scrollbarWidth:"none",msOverflowStyle:"none"}}>
          {categories.map((cat) => (
        ...
        {error ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <button onClick={fetchProducts} className="px-5 py-2 bg-violet-600/20 text-violet-400 text-xs font-bold rounded-full hover:bg-violet-600/30 transition-colors">Try again</button>
          </div>
        ) : loading ? (
      ...
      <section className="relative overflow-hidden mx-4 sm:mx-6 mb-16 rounded-3xl">
        ...
        <div className="relative px-8 py-14 sm:py-16 text-center">
          ...
          <h2 className="text-white text-3xl sm:text-4xl font-black tracking-tight mb-3">Chat with us directly</h2>
          <p className="text-white/30 text-sm mb-8 max-w-sm mx-auto">Order via WhatsApp, ask about sizing, or check if something is back in stock.</p>
          <a href="https://wa.me/+260978917953?text=Hi! I'd like to order." target="_blank" rel="noreferrer"
